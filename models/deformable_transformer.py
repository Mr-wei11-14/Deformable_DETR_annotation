...
class DeformableTransformer(nn.Module):
    ...
    def gen_encoder_output_proposals(self, memory, memory_padding_mask, spatial_shapes):
        ...
        base_scale = 4.0  # 定义了一个基础缩放因子，用于后续计算提议框的大小。但是没用到
        proposals = []
        ...
        for lvl, (H_, W_) in enumerate(spatial_shapes):
            ...
            # (2**i)使得每个特征层的提议框大小与其在特征金字塔中的层级成正比。
            # 0.05 意味着提议框的起始宽度和高度设置为特征图尺寸的 5%。
        ...
        # 对提议框坐标应用 logit 函数，这是 Sigmoid 函数的逆。
        ...
        # 对encoder输出进行处理：经过一层全连接层 + LayerNorm
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            # scale-level position embedding  [bs,h*w,c] + [1,1,c] -> [bs,h*w,c]
        ...
        if self.two_stage:
            ...
            # 多分类：[bs, H/8 * W/8 + ... + H/64 * W/64, 256] -> [bs, H/8 * W/8 + ... + H/64 * W/64, 91] （相当于每个像素点的分类）
            ...
            # top-k proposal box进行处理得到最终的query和query pos
            # self.pos_trans = nn.Linear(d_model * 2, d_model * 2)
            # self.pos_trans_norm = nn.LayerNorm(d_model * 2)
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # self attention  这里的self_attn为多尺度自注意力
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        """
        生成参考点   reference points  为什么参考点是中心点？ 
        spatial_shapes: 4个特征图的shape [4, 2]
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        ...
        for _, layer in enumerate(self.layers):
            # 最好能够在encoder过程中更新reference_points或者在encoder之后更新proposal
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    @staticmethod
    def with_pos_embed(tensor, pos):  # 加上位置信息
        return tensor if pos is None else tensor + pos
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        ...
        # 也就是query自己attention，其实没啥用，mask2former中说先cross-attention会好一点
        ...
        # 所以q=query embedding + query pos, v=上一层输出的output
        # cross attention
...
class DeformableTransformerDecoder(nn.Module):            
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        """
        tgt: 预设的query embedding [bs, 300, 256]
        query_pos: 预设的query pos [bs, 300, 256] 也就是之前的query embed, 因为参考点都是随机生成的，所以query pos也是随机的
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if reference_points.shape[-1] == 4:
                ...
                # 这里的reference_points是通过query_embed生成的(bs, num, 2)
            ...
            if self.bbox_embed is not None:
                tmp = self.bbox_embed[lid](output)  # lid 第几层decoder [bs, 300, 256] -> [bs, 300, 4(xywh)]
                if reference_points.shape[-1] == 4:
